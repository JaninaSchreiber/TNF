...
class metrics:
    ...
    def calculate_robustness(self, feasible_point, instance_directory):
        ...
        #self.hall_of_fame.to_csv(instance_directory + "hall_of_fame.csv")
...
def summarize_results(met, ts_all, te_all, pathname):
    instance_dir = PATH
    ...
    directory = instance_dir + pathname
    print('output directory {}'.format(directory)) 
    if not os.path.exists(directory):
        os.makedirs(directory)
    path, dirs, files = next(os.walk(directory))
    print("dirs {}, files {}".format(dirs, files))
    file_count = len(dirs)
    directory = directory + "/test{}".format(file_count+1)
    if not os.path.exists(directory):
        os.mkdir(directory)
    df.to_csv(directory + "/summary.csv")
    met.write_results(directory)
    met.calculate_robustness(90, directory)
