...
// below
(void o) start (int ME_rank, int num_iterations, int num_variations, string ga_parameters_file, string executable_model, string default_xml) {
    ...
    string algo_params = "%d,'%s'" %  (num_iterations, ga_parameters_file);
    EQPy_init_package(ME,"random_search") =>
    EQPy_get(ME) =>
...
main() {
  ...
  start(rank, num_iterations, num_variations, ga_parameters_file, executable, default_xml);
}
